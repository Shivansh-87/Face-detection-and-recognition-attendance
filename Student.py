...
class Student:
    def __init__(self, root):
        ...
        # Title Label
        title_lbl = Label(bg_img, text="PERSONAL DETAILS", font=("times new roman", 35, "bold"), bg="white", fg="red")
        title_lbl.place(x=0, y=0, width=1530, height=45)  # Set width to fit the screen, height for title visibility
        
        main_frame=Frame(bg_img,bd=2,bg="white")
        main_frame.place(x=0,y=45,width=1530,height=650)
        ...
        img_left = Image.open(r"C:\Users\shiva\OneDrive\Desktop\frams\college_images\WhatsApp Image 2024-12-23 at 11.30.23_6d7e79c3.jpg")
        img_left = img_left.resize((750, 130), Image.Resampling.LANCZOS)  
        self.photoimg_left = ImageTk.PhotoImage(img_left)

        bg_img_left = Label(Left_frame, image=self.photoimg_left)
        bg_img_left.place(x=5, y=0, width=750, height=130)
        ...
        # Correct column alignment for widgets
        ...
        course_combo["values"] = ("Select Courses", "FE", "IT", "BE")
        course_combo.current(0)
        course_combo.grid(row=0, column=3, padx=2, pady=10, sticky=W)

        # Adjust year_combo and semester_combo column placement
        ...
        semester_combo["values"] = ("Semester", "1", "2", "3")
        ...
        class_div_combo = ttk.Combobox(class_student_frame,textvariable=self.var_div, font=("times new roman", 12, "bold"), state="readonly", width=19)
        class_div_combo["values"] = ("Select","Gryffindor", "Hufflepuff", "Ravenclaw","Slytherin")
        ...
        # Creating StringVar for radio buttons
        self.photo_sample_var = StringVar()

        # Radio buttons
        radiobtn1 = ttk.Radiobutton(class_student_frame,variable=self.var_radio1, text="Take photo sample", value="Yes")#, variable=self.photo_sample_var)
        radiobtn1.grid(row=6, column=0)

        radiobtn2 = ttk.Radiobutton(class_student_frame,variable=self.var_radio1, text="No photo sample", value="No")#, variable=self.photo_sample_var)
        ...
        save_btn = Button(btn_frame, text="Save",command=self.add_data, width=17, font=("times", 13, "bold"), bg="blue", fg="white")
        save_btn.grid(row=0, column=0)

        update_btn = Button(btn_frame, text="Update",command=self.update_data, width=18, font=("times", 13, "bold"), bg="blue", fg="white")
        update_btn.grid(row=0, column=1)

        delete_btn = Button(btn_frame, text="Delete",command=self.delete_data, width=18, font=("times", 13, "bold"), bg="blue", fg="white")
        delete_btn.grid(row=0, column=2)

        reset_btn = Button(btn_frame, text="Reset",command=self.reset_data, width=17, font=("times", 13, "bold"), bg="blue", fg="white")
        ...
        take_btn = Button(btn_frame1,command=self.generate_dataset ,text="Take Photo Sample", width=36, font=("times", 13, "bold"), bg="blue", fg="white")
        take_btn.grid(row=0, column=0)

        update_photo_btn = Button(btn_frame1, text="Update Photo Sample", width=36, font=("times", 13, "bold"), bg="blue", fg="white")
        ...
        #image
        img_right = Image.open(r"C:\Users\shiva\OneDrive\Desktop\frams\college_images\WhatsApp Image 2024-12-23 at 11.30.23_6d7e79c3.jpg")
        img_right = img_right.resize((750, 130), Image.Resampling.LANCZOS)  
        self.photoimg_right = ImageTk.PhotoImage(img_right)

        bg_img_left = Label(Right_frame, image=self.photoimg_left)
        bg_img_left.place(x=5, y=0, width=750, height=130)
        ...
        search_combo = ttk.Combobox(Serach_frame, font=("times new roman", 12, "bold"), state="readonly", width=15)
        search_combo["values"] = ("Select", "Roll_no", "PhoneNo", "Id")
        search_combo.current(0)
        search_combo.grid(row=0, column=1, padx=2, pady=10, sticky=W)

        search_entry=ttk.Entry(Serach_frame,width=15,font=("times new roman", 12, "bold"))
        search_entry.grid(row=0,column=2,padx=10,pady=5,sticky=W)

        search_btn = Button(Serach_frame, text="Search", width=12, font=("times", 12, "bold"), bg="blue", fg="white")
        search_btn.grid(row=0, column=3,padx=4)

        showAll_btn = Button(Serach_frame, text="Show All", width=12, font=("times", 12, "bold"), bg="blue", fg="white")
        showAll_btn.grid(row=0, column=4,padx=4)

        # ==============table frame=================
        table_frame =Frame(Right_frame, bd=2, bg="white", relief=RIDGE)
        table_frame.place(x=5, y=210, width=710, height=342)
    ...
    def add_data(self):
        if self.var_dep.get()=="Select Department" or self.var_std_name.get()=="" or self.var_std_id.get()==" ":
            messagebox.showerror("Error","Ruko Jara, Kuch Rah Gya",parent=self.root)
        else:
            try:
                ...
                self.fetch_data()
                conn.close()
                messagebox.showinfo("success","Congrats, Hogwards me aapka swagat hai",parent=self.root)
        ...
        if self.var_dep.get()=="Select Departement" or self.var_std_name.get()=="" or self.var_std_id.get()==" ":
            ...
            try:
                ...
                if Update>0:
                    ...
                    if not Update:
                        return
                messagebox.showinfo("Success","Student details successfully",parent=self.root)
                conn.commit()
                self.fetch_data()
                conn.close()
            except Exception as es:
...
# ============Take photo sample===========
            ...
            # Ensure "data" directory exists
                        ...
                        if cv2.waitKey(1) == 13 or img_id == 100:  # Press "Enter" to exit or collect 100 images
                            break
